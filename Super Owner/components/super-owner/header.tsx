...
import { Bell, Shield, Check, Search } from "lucide-react"
...
export function useSuperOwnerHeader() {
  const context = useContext(HeaderContext)
  if (!context) throw new Error('useSuperOwnerHeader must be used within SuperOwnerHeaderProvider')
  return context
}

export function SuperOwnerHeaderProvider({ children }: { children: React.ReactNode }) {
  const [notifications, setNotifications] = useState([
    { id: 1, type: 'warning' as const, title: 'Manager on Leave', message: 'John Smith is on leave. Claims auto-routed to you.', time: '2 hours ago', read: false },
    { id: 2, type: 'success' as const, title: 'Claim Approved', message: 'Finance claim #1234 approved successfully.', time: '5 hours ago', read: false },
    { id: 3, type: 'info' as const, title: 'New Escalation', message: '3 claims require your attention.', time: '1 day ago', read: true },
  ])
  ...
  // Real-time subscription for escalation changes
  useEffect(() => {
    let mounted = true
    ...
    const setupSubscription = async () => {
      try {
        ...
        channel = supabase
          .channel('super-owner-claims-changes')
          .on(
            'postgres_changes',
            {
              event: 'UPDATE',
  ...
  return (
    <HeaderContext.Provider value={{ notifications, setNotifications, refreshNotifications }}>
      {children}
...
export function Header({ title, showBackButton, showSearch = false, userRole = 'Super Owner', userId = 'SO-001', userName = 'Admin User' }: HeaderProps) {
  const { notifications, setNotifications } = useSuperOwnerHeader()
