...
export default function EscalationsPage() {
  ...
  useEffect(() => {
    async function fetchEscalatedClaims() {
      try {
        ...
        const { data: profiles, error: profilesError } = await supabase
          .from('profiles')
          .select('id, user_id, full_name, employee_id, on_leave, role')
        ...
        // Scenario B: Manager is on leave and claim is not yet approved
        const onLeaveManagerIds = (profiles || [])
          .filter(p => p.on_leave === true)
          .map(p => p.id)

        let escalatedClaims: any[] = []
        if (onLeaveManagerIds.length > 0) {
          ...
          escalatedClaims = data || []
        }

        // Filter out manager/finance self-claims from escalation (they belong in Pending Approvals)
        const finalEscalations = escalatedClaims.filter((claim: any) => {
          const profile = profileByUserIdMap.get(claim.employee_id) || profileByIdMap.get(claim.employee_id)
          return profile?.role !== 'manager' && profile?.role !== 'finance'
        })

        const transformedClaims = finalEscalations.map((claim: any) => {
          const createdDate = new Date(claim.created_at)
          const now = new Date()
          const daysPending = Math.floor((now.getTime() - createdDate.getTime()) / (1000 * 60 * 60 * 24))
          
          const employeeProfile = profileByUserIdMap.get(claim.employee_id) || profileByIdMap.get(claim.employee_id)
          const managerProfile = claim.manager_id
            ? (profileByUserIdMap.get(claim.manager_id) || profileByIdMap.get(claim.manager_id))
            : null
          ...
          const employeeIdStr = employeeProfile?.employee_id || (claim.employee_id?.length > 10 ? `${claim.employee_id.slice(0, 8)}...` : claim.employee_id) || 'N/A'
          const managerName = managerProfile?.full_name || 'N/A'

          return {
            id: claim.id,
            claimNumber: claim.claim_number || claim.id.slice(0, 8).toUpperCase(),
            ...
            status: mappedStatus,
            escalationReason: 'Manager on leave',
    ...
    // Realtime subscription for escalations page
    const channel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Claims changed, refreshing escalations...')
          fetchEscalatedClaims()
        }
      )
      .subscribe()
