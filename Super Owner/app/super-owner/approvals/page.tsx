...
export default function SuperOwnerApprovalsPage() {
  ...
  useEffect(() => {
    async function fetchPendingApprovals() {
      try {
        // Fetch claims with approval_tier >= 2 (manager/finance self-submitted claims)
        const { data: tierClaims, error: tierError } = await supabase
          .from('claims')
          .select('*')
          .gte('approval_tier', 2)
          .order('created_at', { ascending: false })

        // Also fetch profiles to get names and do role-based fallback
        const { data: profiles, error: profilesError } = await supabase
          .from('profiles')
          .select('id, user_id, full_name, employee_id, role')

        if (profilesError) throw profilesError

        // Build profile maps (by id and user_id)
        ...
        // Also fetch all claims to do role-based filtering (catches claims without approval_tier set)
        ...
        // Merge: use tier-based + role-based, deduplicate by id
        const claimMap = new Map<string, any>()

        // Add tier-based claims
        ;(tierClaims || []).forEach(c => claimMap.set(c.id, c))

        // Add role-based claims (manager/finance who submitted)
        ;(allClaims || []).forEach((claim: any) => {
          const profile = profileByUserIdMap.get(claim.employee_id) || profileByIdMap.get(claim.employee_id)
          const isManagerOrFinance = profile?.role === 'manager' || profile?.role === 'finance' || profile?.role === 'super_owner'
          if (isManagerOrFinance) claimMap.set(claim.id, claim)
        })

        const mergedClaims = Array.from(claimMap.values())

        const transformedClaims = mergedClaims.map((claim: any): PendingApproval => {
          ...
          if (profile?.role === 'manager') {
            type = 'manager'
            reason = 'Manager claim — requires Super Owner approval'
          } else if (profile?.role === 'finance') {
            type = 'finance'
            reason = 'Finance claim — requires Super Owner approval'
          } else if (claim.approval_tier >= 2) {
            reason = 'Escalated — requires Super Owner approval'
          }
          
          let mappedStatus = (claim.status || '').toLowerCase()
          if (mappedStatus === 'submitted') mappedStatus = 'pending'
          
          // Better fallbacks for missing profile data
          const claimantName = profile?.full_name || 'Unknown'
          const claimantId = profile?.employee_id || (claim.employee_id?.length > 10 ? `${claim.employee_id.slice(0, 8)}...` : claim.employee_id) || 'N/A'
          const claimantRole = profile?.role || (claim.approval_tier >= 2 ? 'Manager' : 'Employee')
    ...
    // Realtime subscription for approvals page
    const channel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Claims changed, refreshing approvals...')
          fetchPendingApprovals()
        }
      )
      .subscribe()
