...
function CreateClaimPageContent() {
  ...
  const handleSubmit = async (isDraft: boolean = false) => {
    ...
    try {
      ...
      // Upload receipt to Storage if file exists
      let receiptUrl = ''
      if (receiptFile) {
        try {
      ...
      console.log('Creating claim with input:', input)
      await createClaim(input)
      
      router.push(isEditMode ? '/dashboard/claims' : '/dashboard/claims')
    } catch (err) {
  ...
  return (
    <DashboardLayout title="Create Claim" showSearch={false}>
...
function CreateClaimPageFallback() {
  return (
    <DashboardLayout title="Create Claim" showSearch={false}>
...
export default function CreateClaimPage() {
  return (
    <Suspense fallback={<CreateClaimPageFallback />}>
      <CreateClaimPageContent />
    </Suspense>
  )
}
