...
import { ArrowLeft, CheckCircle, XCircle, Clock, User, DollarSign, Calendar, FileText, Receipt, AlertTriangle, UserMinus, Loader2 } from "lucide-react"
...
export default function EscalationReviewPage() {
  ...
  const handleApprove = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        alert("Escalated claim approved. Routed to Finance for payment.")
      } else {
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      alert("Escalated claim rejected. Reason: " + reason)
    } catch (error) {
  ...
  return (
    <div className="p-8">
      <Link href="/super-owner/escalations" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary mb-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-muted-foreground mb-3">{employeeId} • {department}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-6">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-lg">Manager on Leave</h3>
                <p className="text-sm text-muted-foreground mb-2">Original Approver</p>
                <div className="bg-destructive/10 rounded-lg p-3 flex items-start gap-2">
                  <AlertTriangle className="w-4 h-4 text-destructive mt-0.5" />
                  <div>
                    <p className="text-sm font-medium text-destructive">Manager on leave</p>
                    <p className="text-xs text-muted-foreground mt-1">Auto-routed to Super Owner on {createdAt}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-destructive/10 flex items-center justify-center"><AlertTriangle className="w-4 h-4 text-destructive" /></div>
                <div><p className="text-sm font-medium text-destructive">Escalated</p><p className="text-xs text-muted-foreground">{createdAt}</p></div>
              </div>
              <div className="w-px h-6 bg-border ml-4" />
              <div className="flex items-center gap-3">
          ...
          <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">Auto-routed</span><span className="font-medium text-primary">Yes</span></div>
            </div>
          </div>
        </div>
      </div>
