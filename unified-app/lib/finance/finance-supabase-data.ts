...
async function loadEmployeeLookupForClaims(
  ...
  if (error) {
    console.warn('Unable to load manager team mapping for finance claims:', error)
    return lookup
  }

  for (const row of (data || []) as TeamDirectoryRow[]) {
    const employeeUserId = String(row.employee_user_id || '')
    if (!employeeUserId) continue

    lookup.set(employeeUserId, {
      employeeId: row.employee_id || employeeUserId,
...
export async function fetchPendingVerification(): Promise<FinanceClaim[]> {
  const allClaims = await fetchAllClaims()
  return allClaims.filter(c => c.status === 'Approved')
}

// Fetch approved claims ready for payment
export async function fetchReadyForPayment(): Promise<FinanceClaim[]> {
  const allClaims = await fetchAllClaims()
  // Support both explicit finance status and legacy schemas that still use Approved.
  return allClaims.filter(c => c.status === 'PaymentProcessing' || c.status === 'Approved')
}
...
export async function fetchFinanceProfile(): Promise<FinanceProfile> {
  ...
  if (error) {
    ...
    return mapProfile(dataById as DbProfile)
  }
...
export async function buildFinanceDashboardStats(): Promise<FinanceDashboardStats> {
  const allClaims = await fetchAllClaims()
  const pendingVerification = allClaims.filter((claim) => claim.status === 'Approved')
...
export async function financeApproveClaim(claimId: string): Promise<void> {
  ...
  if (error) {
    ...
    if (isStatusConstraintError) {
      // Backward-compatible fallback for databases that don't yet allow PaymentProcessing.
