...
export async function fetchCurrentProfile(): Promise<DbProfile> {
  ...
  if (!teamDepartment && !reportingManagerName && finalData.employee_id) {
    try {
  ...
  return {
    ...
    phone:
      finalData.phone ??
      (typeof (user.user_metadata as any)?.phone === 'string' ? (user.user_metadata as any).phone : null),
    department: teamDepartment || finalData.department,
    manager_name: reportingManagerName || finalData.manager_name || null,
  }
}
...
export async function createClaim(input: CreateClaimInput): Promise<DashboardClaim> {
  ...
  const blockedVendors: string[] = [] // No blocked vendors by default
...
export async function updateProfile(input: Partial<DbProfile>): Promise<DbProfile> {
  ...
  const currentMetadataPhone =
    typeof (user.user_metadata as any)?.phone === 'string' && (user.user_metadata as any).phone.trim()
      ? (user.user_metadata as any).phone.trim()
      : null
  ...
  if (input.phone !== undefined) {
    ...
    if (nextPhone !== currentMetadataPhone) {
      ...
      metadataPhone =
        typeof (authData.user?.user_metadata as any)?.phone === 'string' &&
        (authData.user.user_metadata as any).phone.trim()
          ? (authData.user.user_metadata as any).phone.trim()
          : null
    }
  }
