...
import { fetchClaimById, approveClaim, rejectClaim, requireUser, type ManagerClaim } from "@/lib/dashboard/manager-supabase-data"
...
function getStatusHeadline(status: string) {
  ...
  if (status === "Paid") return "Finance Processed"
  return "Draft"
}
...
export default function ClaimDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [claim, setClaim] = useState<ManagerClaim | null>(null)
  ...
  const isReviewPending = claim?.status === "Pending" || claim?.status === "Submitted"
  const isBlobUrl = claim?.receiptUrl?.startsWith('blob:')
  const isSelfApproval = currentUserId && claim?.employeeId === currentUserId
  ...
  const timelineSteps = useMemo(() => {
    ...
    return [
      ...
      {
        label: isSelfApproval ? "Super Owner Review" : "Manager Approval",
        date: managerDone ? formatDateTime(claim.createdAt) : "Waiting...",
        complete: managerDone,
        note: isSelfApproval 
          ? "Routed to Super Owner (self-approval not allowed)" 
          : claim.status === "Approved" 
            ? "Approved in manager review flow." 
            : undefined,
      },
      {
  ...
  }, [claim, isSelfApproval])

  const handleApprove = async () => {
    try {
      setIsProcessing(true)
      await approveClaim(id, reviewNote)
  ...
  const handleReject = async () => {
    try {
      setIsProcessing(true)
      await rejectClaim(id, reviewNote)
  ...
  if (loading) {
    return (
      <ManagerLayout title="Review Claim" showBackButton>
            ...
            <div className="lg:col-span-5 space-y-8">
              {isReviewPending && (
                <div className="bg-card rounded-xl border border-border p-5">
                  <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground mb-3">Manager Action</h3>
                  
                  {isSelfApproval ? (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start gap-3">
                      <AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5 flex-shrink-0" />
                      <div>
                        <p className="text-sm font-semibold text-amber-800">Self-Approval Not Allowed</p>
                        <p className="text-xs text-amber-700 mt-1">You cannot approve your own claim. This claim has been routed to Super Owner for review.</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="flex gap-2">
                        <Button className="flex-1 bg-emerald-600 hover:bg-emerald-700" onClick={handleApprove} disabled={isProcessing}>
              ...
              <div className="pt-8 border-t">
                ...
                <div className="bg-muted/50 p-5 rounded-xl relative">
                  ...
                  <p className="text-xs text-foreground leading-relaxed">
                    This claim is synced from approvals data. Update decision from the Manager Action panel.
                  </p>
                </div>
              </div>
            </div>
